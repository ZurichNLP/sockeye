...
ENCODER_DECODER_SETTINGS = [
    # Sentencepiece sampling - "vanilla" LSTM encoder-decoder with attention
    ("--sentencepiece --sentencepiece-alpha 0.1 --sentencepiece-nbest 64 --min-num-epochs 2"
     " --encoder rnn --decoder rnn --num-layers 1 --rnn-cell-type lstm --rnn-num-hidden 16"
     " --num-embed 16 --rnn-attention-type mlp --rnn-attention-num-hidden 16 --batch-size 13 --loss cross-entropy"
     " --optimized-metric perplexity --max-updates 20 --checkpoint-frequency 10 --optimizer adam --initial-learning-rate 0.01",
     "--beam-size 2",
     False, False, False),]
...
@pytest.mark.parametrize("train_params, translate_params, restrict_lexicon, use_prepared_data, use_source_factors",
                         ENCODER_DECODER_SETTINGS)
def test_sentencepiece(train_params: str,
    ...
    with tmp_digits_dataset(prefix="test_sentencepiece_train_translate",
                            ...
                            sort_target=False,
                            with_sentencepiece_model=True) as data:
        ...
        train_params += " --sentencepiece-model %s" % data['sentencepiece_model_path']
