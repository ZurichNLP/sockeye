...
class EarlyStoppingTrainer:
    ...
    def fit(self,
        ...
        while True:
            ...
            # CHECKPOINT
            ############
            if self.state.updates > 0 and self.state.updates % (checkpoint_interval * self.update_interval)  == 0:
...
class Speedometer:
    ...
    def __call__(self, epoch: int, batches: int, updates: int, samples: int, tokens: int, metric: Optional[mx.metric.EvalMetric], metric_cosine_loss: Optional[mx.metric.EvalMetric]):
        count = batches
